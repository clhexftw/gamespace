...
public class DataUsageSummary extends DataUsageBaseFragment implements DataUsageEditController {
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (isGuestUser(context)) {
            Log.e("DataUsageSummary", "This setting isn't available due to user restriction.");
            EventLog.writeEvent(1397638484, "262243574", -1, "Guest user");
            finish();
        } else if (!isSimHardwareVisible(context)) {
            finish();
        } else {
            enableProxySubscriptionManager(context);
            boolean hasMobileData = DataUsageUtils.hasMobileData(context);
            int defaultDataSubscriptionId = SubscriptionManager.getDefaultDataSubscriptionId();
            boolean z = defaultDataSubscriptionId != -1 ? hasMobileData : false;
    ...
    static CharSequence formatUsage(Context context, String str, long j, float f, float f2) {
        ...
        CharSequence expandTemplate = TextUtils.expandTemplate(new SpannableString(context.getString(17040337).replace("%1$s", "^1").replace("%2$s", "^2")), spannableString, formatBytes.units);
