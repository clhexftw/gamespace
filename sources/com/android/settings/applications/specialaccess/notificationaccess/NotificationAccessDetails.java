...
public class NotificationAccessDetails extends DashboardFragment {
    ...
    private boolean hasInteractAcrossUsersPermission() {
        ...
        if (TextUtils.isEmpty(initialCallingPackage)) {
            Log.w("NotifAccessDetails", "Not able to get calling package name for permission check");
            return false;
        } else if (getContext().getPackageManager().checkPermission("android.permission.INTERACT_ACROSS_USERS_FULL", initialCallingPackage) != 0) {
            Log.w("NotifAccessDetails", "Package " + initialCallingPackage + " does not have required permission android.permission.INTERACT_ACROSS_USERS_FULL");
            return false;
        } else {
            return true;
        }
