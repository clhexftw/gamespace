...
public class OptimizedChargeFragment extends DashboardFragment {
    ...
    public String getLogTag() {
        return "OptimizedChargeFragment";
    }
    ...
    private final class SettingsObserver extends ContentObserver {
        private final Uri mCeilingUri;
        private final Uri mEnabledUri;
        private final Uri mFloorUri;
        private final Uri mStatusUri;
        private final Uri mTimeUri;

        SettingsObserver() {
            super(new Handler());
            this.mCeilingUri = Settings.System.getUriFor("optimized_charge_ceiling");
            this.mEnabledUri = Settings.System.getUriFor("optimized_charge_enabled");
            this.mFloorUri = Settings.System.getUriFor("optimized_charge_floor");
            this.mStatusUri = Settings.System.getUriFor("optimized_charge_status");
            this.mTimeUri = Settings.System.getUriFor("optimized_charge_time");
        }

        void register(ContentResolver contentResolver) {
            contentResolver.registerContentObserver(this.mCeilingUri, false, this);
            contentResolver.registerContentObserver(this.mEnabledUri, false, this);
            contentResolver.registerContentObserver(this.mFloorUri, false, this);
        ...
        public void onChange(boolean z, Uri uri) {
            OptimizedChargeFragment.this.updatePreferenceStates();
        }
    }
    ...
    public Dialog onCreateDialog(int i) {
        ...
        int parseInt2;
        if (i == 0 || i == 1) {
            String[] split = OptimizedChargingSettingsHelper.getOptimizedChargingTime(getContext()).split(",");
            final boolean z = i == 0;
            TimePickerDialog.OnTimeSetListener onTimeSetListener = new TimePickerDialog.OnTimeSetListener() { // from class: com.android.settings.fuelgauge.OptimizedChargeFragment$$ExternalSyntheticLambda0
                @Override // android.app.TimePickerDialog.OnTimeSetListener
                public final void onTimeSet(TimePicker timePicker, int i2, int i3) {
                    OptimizedChargeFragment.this.lambda$onCreateDialog$0(z, timePicker, i2, i3);
                }
            };
            if (z) {
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if ("optimized_charge_start_time".equals(preference.getKey())) {
            writePreferenceClickMetric(preference);
            showDialog(0);
            return true;
        } else if ("optimized_charge_end_time".equals(preference.getKey())) {
            writePreferenceClickMetric(preference);
            showDialog(1);
            return true;
        } else {
    ...
    private void updateTimeSetting(boolean z, int i, int i2) {
        ...
        OptimizedChargingSettingsHelper.setOptimizedChargingTime(getContext(), split[0] + "," + split[1]);
    }
}
