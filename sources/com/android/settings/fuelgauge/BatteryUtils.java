...
public class BatteryUtils {
    ...
    public static int getCurrentDockDefenderMode(Context context, BatteryInfo batteryInfo) {
        if (batteryInfo.pluggedStatus == 8) {
            if (Settings.Global.getInt(context.getContentResolver(), "dock_defender_bypass", 0) == 1) {
                return 2;
            }
            if (batteryInfo.isOverheated && FeatureFactory.getFactory(context).getPowerUsageFeatureProvider(context).isExtraDefend()) {
                return 1;
            }
            return !batteryInfo.isOverheated ? 0 : 3;
        }
        return 3;
    }
}
