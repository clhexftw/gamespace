...
public class AdvancedPowerUsageDetail extends DashboardFragment implements ButtonActionDialogFragment.AppButtonsDialogListener, BatteryTipPreferenceController.BatteryTipListener, SelectorWithWidgetPreference.OnClickListener {
    ...
    public static void startBatteryDetailPage(Activity activity, Instrumentable instrumentable, String str, UserHandle userHandle) {
        ...
        new SubSettingLauncher(activity).setDestination(AdvancedPowerUsageDetail.class.getName()).setTitleRes(R.string.battery_details_title).setArguments(bundle).setSourceMetricsCategory(instrumentable.getMetricsCategory()).setUserHandle(userHandle).launch();
    }
    ...
    public void onRadioButtonClicked(SelectorWithWidgetPreference selectorWithWidgetPreference) {
        ...
        this.mBatteryOptimizeUtils.setAppUsageState(getSelectedPreference());
    }
    ...
    private CharSequence getAppActiveTime(Bundle bundle) {
        ...
        int i = bundle.getInt("extra_uid", 0);
        String string = bundle.getString("extra_slot_time", null);
        long j3 = j + j2;
        if (FeatureFactory.getFactory(getContext()).getPowerUsageFeatureProvider(getContext()).isChartGraphEnabled(getContext()) || !BatteryEntry.isSystemUid(i)) {
            if (j3 == 0) {
