...
public class SubscriptionsPreferenceController extends AbstractPreferenceController implements LifecycleObserver, SubscriptionsChangeListener.SubscriptionsChangeListenerClient, MobileDataEnabledListener.Client, DataConnectivityListener.Client, SignalStrengthListener.Callback, TelephonyDisplayInfoListener.Callback, TelephonyCallback.CarrierNetworkListener {
    ...
    Drawable getIcon(int i) {
        ...
        if (isRegistered || z || isCarrierNetworkActive) {
            drawable = this.mSubsPrefCtrlInjector.getIcon(this.mContext, i3, i4, !createForSubscriptionId.isDataEnabled(), this.mCarrierNetworkChangeMode);
        }
        if (this.mSubsPrefCtrlInjector.isActiveCellularNetwork(this.mContext) || isCarrierNetworkActive) {
    ...
    void setIcon(Preference preference, int i, boolean z) {
        ...
        preference.setIcon(this.mSubsPrefCtrlInjector.getIcon(this.mContext, level, i2, (z && createForSubscriptionId.isDataEnabled()) ? true : true, this.mCarrierNetworkChangeMode));
    }
    ...
    public static class SubsPrefCtrlInjector {
        ...
        public String getNetworkType(Context context, MobileMappings.Config config, TelephonyDisplayInfo telephonyDisplayInfo, int i, boolean z, boolean z2) {
            ...
            if (z) {
                signalIcon$MobileIconGroup = TelephonyIcons.CARRIER_MERGED_WIFI;
            } else if (z2) {
                signalIcon$MobileIconGroup = TelephonyIcons.CARRIER_NETWORK_CHANGE;
            } else {
                signalIcon$MobileIconGroup = MobileMappings.mapIconSets(config).get(MobileMappings.getIconKey(telephonyDisplayInfo));
            }
            if (signalIcon$MobileIconGroup == null) {
                Log.d("SubscriptionsPrefCntrlr", "Can not get the network's icon and description.");
                return "";
            }
