...
public class MobileNetworkUtils {
    ...
    public static boolean isMobileNetworkUserRestricted(Context context) {
        ...
        if (userManager != null) {
            return userManager.isGuestUser() || userManager.hasUserRestriction("no_config_mobile_networks");
        }
        return false;
    }
    ...
    public static Drawable getSignalStrengthIcon(Context context, int i, int i2, int i3, boolean z, boolean z2) {
        int state;
        Drawable drawable;
        SignalDrawable signalDrawable = new SignalDrawable(context);
        if (z2) {
            state = SignalDrawable.getCarrierChangeState(i2);
        } else {
            state = SignalDrawable.getState(i, i2, z);
        }
    ...
    private static boolean isCurrentCountrySupported(Context context) {
        ...
        for (int i = 0; i < telephonyManager.getPhoneCount(); i++) {
            ...
            if (!TextUtils.isEmpty(networkCountryIso)) {
                hashSet.add(networkCountryIso);
            }
        }
        Stream stream = hashSet.stream();
        Objects.requireNonNull(euiccManager);
        boolean anyMatch = stream.anyMatch(new Predicate() { // from class: com.android.settings.network.telephony.MobileNetworkUtils$$ExternalSyntheticLambda1
            @Override // java.util.function.Predicate
            public final boolean test(Object obj) {
                return euiccManager.isSupportedCountry((String) obj);
            }
        });
        Log.i("MobileNetworkUtils", "isCurrentCountrySupported countryCodes: " + hashSet + " eSIMSupported: " + anyMatch);
        return anyMatch;
    }
